...
def main(initial_path):
    ...
    nrm_path = os.path.join(samples_path['Meshes'], 'Additional_maps')
    ...
    batchtools.bake_normal_from_meshes(exe_path['substance3d_baker'], samples_path['Meshes'], '.FBX', '11', '11', output_path['Meshes'])

    # Bake ID maps from meshes detected (will collect low/high poly based on '_high' and '_low' suffix)
    batchtools.bake_id_from_meshes(exe_path['substance3d_baker'], samples_path['Meshes'], '.FBX', '11', '11', output_path['Meshes'])

    # Bake ao from low poly meshes detected (will collect .obj) and Normal maps (the mesh name must be the same)
    batchtools.bake_from_meshes_and_nrm(exe_path['substance3d_baker'], 'ambient-occlusion', samples_path['Meshes'], '.FBX', nrm_path, '.png', '11', '11', output_path['Meshes'])

    # Bake curvature from low poly meshes detected (will collect .obj) and Normal maps (the mesh name must be the same)
    batchtools.bake_from_meshes_and_nrm(exe_path['substance3d_baker'], 'curvature', samples_path['Meshes'], '.FBX', nrm_path, '.png', '11', '11', output_path['Meshes'])

    # Bake world space normal from low poly meshes detected (will collect .obj) and Normal maps
    # (the mesh name must be the same)
    batchtools.bake_from_meshes_and_nrm(exe_path['substance3d_baker'], 'normal-world-space', samples_path['Meshes'], '.FBX', nrm_path, '.png', '11', '11', output_path['Meshes'])
