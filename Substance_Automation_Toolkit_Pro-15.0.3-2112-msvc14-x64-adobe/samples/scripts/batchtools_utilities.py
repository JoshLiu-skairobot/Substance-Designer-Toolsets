...
def render_sbsar(sbsrender_path, sbsar_file_path, width_resolution, height_resolution, output_path, debug=True):
    # Classic cooking (no error log)
    cmd = [sbsrender_path, "render", "--inputs", sbsar_file_path, "--set-value", "$outputsize@" + width_resolution + "," + height_resolution, "--output-path", output_path]
    if debug:
...
def render_sbsar_files(sbsrender_path, sbsar_files_path, width_resolution, height_resolution, output_path, debug=True):
    files_to_process = get_files_by_extension(sbsar_files_path, ".sbsar", debug)
    if files_to_process:
        for sbsar in files_to_process:
            render_sbsar(sbsrender_path, sbsar, width_resolution, height_resolution, output_path, debug)
    else:
...
# Export sbs with dependencies
def export_sbs_with_dep(sbsmutator_path, sd_resources_path, sbs_file_path, output_path, debug=True):
    # Classic cooking (no error log)
    export_cmd = [sbsmutator_path, "export", "--input", sbs_file_path, "--presets-path", sd_resources_path,"--preserve-alias", "\"sbs\"", "--output-path", output_path]
    if debug:
        print("____________________")
...
def export_sbs_files_with_dep(sbsmutator_path, sd_resources_path, sbs_files_path, output_path, debug=True):
    files_to_process = get_files_by_extension(sbs_files_path, ".sbs", debug)
    if files_to_process:
        for sbs in files_to_process:
            export_sbs_with_dep(sbsmutator_path, sd_resources_path, sbs, output_path, debug)
    else:
...
# Create sbsar from sbs files using sbscooker
def cook_sbsar(sbscooker_path, sd_resources_path, sbs_file_path, output_path, debug=True):
    # Classic cooking (no error log)
    cook_sbsar_cmd = [sbscooker_path, "--inputs", sbs_file_path, "--includes", sd_resources_path, "--quiet", "--size-limit", "13", "--output-path", output_path]
    if debug:
        print("____________________")
        print("Processing: "+sbs_file_path)
...
def cook_sbsar_files(sbscooker_path, sd_resources_path, sbs_files_path, output_path, debug=True):
    files_to_process = get_files_by_extension(sbs_files_path, ".sbs", debug)
    if files_to_process:
        for sbs in files_to_process:
...
def bake_normal_from_meshes(sbsbaker_path, meshes_path, meshes_extension, width_resolution, height_resolution, output_path, debug=True):
    meshes_dictionary = detect_high_and_low_poly_meshes(meshes_path, meshes_extension, debug)
    for (low_poly, high_poly) in meshes_dictionary.items():
        bake_nrm_cmd = [sbsbaker_path,
                        "normal-from-mesh",
                        "--inputs", low_poly,
                        "--highdef-mesh", high_poly,
                        "--quiet",
...
def bake_normal_from_meshes_match_by_name(sbsbaker_path, meshes_path, meshes_extension, width_resolution, height_resolution, output_path, debug=True):
    meshes_dictionary = detect_high_and_low_poly_meshes(meshes_path, meshes_extension, debug)
    for (low_poly, high_poly) in meshes_dictionary.items():
        bake_nrm_cmd = [sbsbaker_path,
                        "normal-from-mesh",
...
def bake_id_from_meshes(sbsbaker_path, meshes_path, meshes_extension, width_resolution, height_resolution, output_path, debug=True):
    meshes_dictionary = detect_high_and_low_poly_meshes(meshes_path, meshes_extension, debug)
    for (low_poly, high_poly) in meshes_dictionary.items():
        bake_id_cmd = [sbsbaker_path,
                        "color-from-mesh",
                        "--inputs", low_poly,
                        "--highdef-mesh", high_poly,
                        "--color-source", "1",
...
# Bake id maps from meshes detected (will collect .obj and low/high poly based on "_high" and "_low" suffix)
def bake_id_from_meshes_match_by_name(sbsbaker_path, meshes_path, meshes_extension, width_resolution, height_resolution, output_path, debug=True):
    meshes_dictionary = detect_high_and_low_poly_meshes(meshes_path, meshes_extension, debug)
    for (low_poly, high_poly) in meshes_dictionary.items():
        bake_id_cmd = [sbsbaker_path,
                        "color-from-mesh",
...
def bake_from_meshes_and_nrm(sbsbaker_path, baker_name, meshes_path, meshes_extension, nrm_path, nrm_extension, width_resolution, height_resolution, output_path, debug=True):
        ...
        for d in detected_meshes:
            ...
            if "_low" in d:
                ...
                if detected_nrm != None:
                    ...
                    print("No Normal maps detected at: " + str(nrm_path) + " for " + str(baker_name) + " baker.")
        for (low_poly, nrm) in meshes_and_nrm_dictionary.items():
            bake_from_low_cmd = [sbsbaker_path,
                            baker_name,
                            "--inputs", low_poly,
                            "--normal", nrm,
                            "--output-size", width_resolution + "," + height_resolution,
        ...
        for k in detected_meshes_dict:
            ...
            if "_low" in k:
                ...
                if final_dict.get(detected_meshes_dict[k]) == None:
                    ...
                    high_poly_key_name = os.path.splitext(k)[0][:-4] + "_high" + os.path.splitext(k)[1]
                    if detected_meshes_dict.get(high_poly_key_name) != None:
                        # Store the low poly path as key and high poly path as value
                        final_dict[detected_meshes_dict[k]] = detected_meshes_dict[high_poly_key_name]
